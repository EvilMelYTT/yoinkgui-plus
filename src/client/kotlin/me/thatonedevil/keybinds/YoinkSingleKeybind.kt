...
class YoinkSingleKeybind : Key {
    ...
    override fun keyName(): String = "key.yoinkgui-plus.yoinksingle"
    ...
    override fun registerScreenEvents() {
        ScreenEvents.AFTER_INIT.register { _, screen, _, _ ->
            //? if >=1.21.9 {
            ScreenKeyboardEvents.afterKeyPress(screen).register { screen, keyEvent ->
            ...
            //?} else {
            /*ScreenKeyboardEvents.afterKeyPress(screen).register { screen, keyCode, scanCode, modifiers ->
                if (!isValidInventoryScreen(screen)) return@register
                if (!yoinkGuiSettings.enableSingleItemYoink.get()) return@register
